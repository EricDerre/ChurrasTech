...
namespace ERP.TestApp;

class Program
{
    static async Task Main(string[] args)
    {
        Console.WriteLine("=== ChurrasTech ERP - Ambiente Teste ===");
        Console.WriteLine("Sistema de Gestão para Churrascaria - TESTE");
        Console.WriteLine();
    ...
    static IHostBuilder CreateHostBuilder(string[] args) =>
        Host.CreateDefaultBuilder(args)
            ...
            {
                // Configure Entity Framework for test environment (using in-memory for demo)
                services.AddDbContext<ChurrasDbContext>(options =>
                    options.UseInMemoryDatabase("ChurrasTest"));
    ...
    static async Task InitializeDatabase(IHost host)
    {
        ...
        try
        {
            ...
            Console.WriteLine("✓ Banco de dados de teste inicializado com sucesso");
        }
        catch (Exception ex)
        {
